...
import { popView, pushView } from '../../views/actions';
   ...
   render() {
      ...
      return (
         <Container hideTitle={hideTitle && !exiting}>
            ...
            />
            {currentView && currentView.name !== 'Search' && (
               <SearchHeaderButton
                  onClick={() => this.props.pushView({ name: 'Search', title: 'Artists and Songs', props: { hideTitle: true } })}>
                  <span role="img" aria-label="search">🔍</span> Artists & Songs
               </SearchHeaderButton>
            )}
         </Container>
      );
